...
RelocatableInst::SharedPtrVec getExecBlockPrologue() {
    ...
    prologue.push_back(Adr(CPUMode::ARM, Reg(REG_SP), 4080));
...
RelocatableInst::SharedPtrVec getExecBlockEpilogue() {
    ...
    // Save guest state, except LR which was saved by the JmpEpilogue generator
...
RelocatableInst::SharedPtrVec getTerminator(rword address, CPUMode cpuMode) {
    ...
    append(terminator, SaveReg(Reg(2), Offset(Reg(2))).generate(nullptr, 0, 0, cpuMode, nullptr, nullptr));
    terminator.push_back(Ldr(cpuMode, Reg(2), Constant(address)));
    append(terminator, SaveReg(Reg(2), Offset(Reg(REG_PC))).generate(nullptr, 0, 0, cpuMode, nullptr, nullptr));
    append(terminator, LoadReg(Reg(2), Offset(Reg(2))).generate(nullptr, 0, 0, cpuMode, nullptr, nullptr));
